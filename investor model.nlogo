...
NFCs-own [
  ...
  share-repurchasing-spent-capital ;;float
  capital-spent-on-repurchases ;;integer
  spendable-liquidity ;;float
  profit-to-equity-ratio ;;float
  ebit ;;integer
...
to setup-map
  print "Loading map..."
  clear-links
  set us-dataset gis:load-dataset "us_state_modified.shp"  ;load a shapefile of the properties
  ask patches [set world? false]
  gis:set-world-envelope gis:envelope-of us-dataset
  gis:set-transformation gis:world-envelope [-398 398 -224 224]
  ask patches gis:intersecting us-dataset
  [ set pcolor 8
    set world? true
  ]
  set world-patches patches with [world? = true]
  ask patches with [world? = false] [
    set pcolor blue
  ]
end
...
to connect-network
  ...
  foreach gis:feature-list-of us-dataset [ [a] -> ; each polyline
    foreach gis:vertex-lists-of a [ [b] ->; each polyline segment / coordinate pair
      foreach b [ [c] -> ; each coordinate
        let location gis:location-of c
        if not empty? location [ ; some coordinates are empty []
          create-nodes 1 [
            ...
            if previous-node != nobody [
              create-node-link-with previous-node
            ]
...
;  connect adjacent polylines/roads
  ask nodes [
    create-node-links-with other nodes in-radius 0.001
    set hidden? true
  ]
...
to define-globals
  ...
  set stdev-initial-share-price 157.61
  if tax-break-repurchasing? = false [
    set tax-break-repurchasing 0
  ]
  ;; Mean and standard deviation taken from the share prices of the companies in the S&P 500 index
end

to define-investing-strategies
...
to define-interest-table
  ...
  foreach (range 5 16) [
    dti ->
    table:put interest-table dti minimum-interest-rate + (dti - 5) * ((maximum-interest-rate - minimum-interest-rate) / 10)
  ]
end
...
to spawn-nfcs
  create-NFCs number-of-NFCs [
    ...
    ;; Creation of a lognormal distribution for the price of shares (most common for distribution of stock prices)
    let B ln(1 + (stdev-initial-share-price ^ 2 / mean-initial-share-price ^ 2))
    let M ln(mean-initial-share-price) - B / 2
    let S sqrt(B)
    set price-per-share exp (random-normal M S )
    set price-history table:make
    table:put price-history 0 price-per-share

    ;; Creation of a power law for the market capitalization of NFCs (fitted from data of S&P 500 companies)
    let x random-float(500)
    set market-cap 3e10 * x ^ -0.999
    set number-of-outstanding-shares round (market-cap / price-per-share)

    ;;Creation of a logarithmic distribution for the profit to equity ratio of NFCs (fitted from data of S&P 500 companies)
    let w random-float(500)
    set profit-to-equity-ratio 0.8316 - 0.1207 * ln(w)
    set ebit profit-to-equity-ratio * market-cap

    ;; Creation of a distribution for debt-to-equity ratio (fitted from data of S&P 500 companies
    set debt-to-ebit-ratio 5 + random-float 5
    set debt debt-to-ebit-ratio * ebit
    set interest-rate table:get interest-table round(debt-to-ebit-ratio)
    ...
    set dividend? true
    ifelse dividend? = true [
      let d random-float(500)
      set dividend-rate 0.0924 - 0.013 * ln(d)
    ] [ set dividend-rate 0 ]
    ...
    set growth-rate -0.1 + random-float 0.2
    set real-investment-rate  0.1 + random-float 0.2
  ...
  foreach range count NFCs [
    ...
    ask supply-investors [
      ...
      while [matrix:get forecast-matrix firm 3 != 0 and any? demand-investors = true and empty? table:to-list bidding-table = false] [
        ...
        let bid-amount min (list [matrix:get forecast-matrix firm 3] of business-partner abs(matrix:get forecast-matrix firm 3) round([spendable-liquidity] of business-partner / bid-price))
        if bid-amount > 0 [
...
to calculate-return-and-new-price
  ...
  ask NFCs [
    ...
    let new-price price-per-share + lambda-price-adjustment * demand-minus-supply
...
to payout-dividend
  ask NFCs [
    set net-income net-income - dividend-rate * (table:get price-history (ticks - 2) * number-of-outstanding-shares)
...
to determine-investment-and-debt
  ask NFCs [
    ifelse decision-rules = "constant-payout-and-investment" [
      set real-investment-rate percentage-of-net-income-to-investment
      if net-income > 0 [
        set real-investment-rate real-investment-rate + (net-income / table:get net-income-history (ticks - 12))
      ]
      set net-income net-income - (real-investment-rate * net-income)
      if net-income < 0 [
        set debt debt - net-income
      ]
    ]
    [ifelse net-income > 0 [
      set real-investment-rate (net-income / table:get net-income-history (ticks - 12))
      ]
      [ set debt debt - net-income
...
to calculate-new-income
  ifelse tax-break-repurchasing? = true [
    let tax-break 0
    ifelse capital-spent-on-repurchases < (limit-for-tax-break * table:get net-income-history (ticks - 12)) [
      set tax-break tax-break-repurchasing * (capital-spent-on-repurchases * price-per-share)
    ]
    [
      set tax-break 0
    ]
    set ebit ebit * (1 + growth-rate)
    set ebt ebit - (interest-rate * debt)
    set ebt ebt - tax-break
    set total-tax-break total-tax-break + tax-break

    ifelse ebt > 0 [
      set net-income ebt * (1 - tax-profit) + tax-break
    ] [ set net-income ebt ]
  ]
  [ set ebit ebit * (1 + growth-rate)
    set ebt ebit - (interest-rate * debt)
...
to calculate-share-repurchasing-MNI
  ...
  ifelse net-income-difference > 0 [
    set share-repurchasing-programme? true
    set share-repurchasing-target round ( limit-for-tax-break * net-income / price-per-share )
    matrix:set-row forecast-matrix who (list (price-per-share) (price-per-share * 1.1) (0.01) (share-repurchasing-target))
  ] [
    set share-repurchasing-programme? true
    set share-repurchasing-target round ( ( limit-for-tax-break * net-income / price-per-share ) * 0.5 )
    matrix:set-row forecast-matrix who (list (price-per-share) (price-per-share * 1.1) (0.01) (share-repurchasing-target))
  ]
end

to calculate-dividend-rate-CPI
  if net-income > table:get net-income-history (ticks - 12) and table:get net-income-history (ticks - 12) > 0 [
    let net-income-difference net-income - table:get net-income-history (ticks - 12)
    set dividend-rate dividend-rate * (1 + (net-income-difference / table:get net-income-history (ticks - 12)))
  ]
end

to calculate-share-repurchasing-CPI
  set share-repurchasing-programme? true
  set share-repurchasing-target round ( (item 0 percentages-payout-and-investment * net-income) / price-per-share )
  matrix:set-row forecast-matrix who (list (price-per-share) (price-per-share * 1.1) (0.01) (share-repurchasing-target))
end
...
2096
273
Percentage of companies with share repurchasings programs
frequency true [share-repurchasing-programme?] of NFCs / count NFCs * 100
1
1
11
...
2098
323
Capital spent on share repurchases by NFCs
total-capital-spent-on-repurchases
0
1
11
...
225
584
Tax policies on dividends, capital gain and profits
14
0.0
1
...
374
581
Tax break for limiting share repurchases
14
0.0
1
...
"" ""
PENS
"rate0" 1.0 0 -16777216 true "" "plot [dividend-rate] of NFC 0"
"rate1" 1.0 0 -3844592 true "" "plot [dividend-rate] of NFC 1"
...
@#$#@#$#@
<experiments>
  <experiment name="test-scenario-analysis" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="156"/>
    <metric>average-real-investment-rate</metric>
    <enumeratedValueSet variable="excess-demand-factor">
      <value value="200"/>
      <value value="1000"/>
      <value value="2000"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="priority-repurchasing">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="cash-reserve-percentage">
      <value value="0.01"/>
      <value value="0.25"/>
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="lambda-price-adjustment">
      <value value="0.01"/>
      <value value="0.1"/>
      <value value="0.2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="maximum-liquidity-demand">
      <value value="0.05"/>
      <value value="0.15"/>
      <value value="0.2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="minimum-liquidity-demand">
      <value value="0.25"/>
      <value value="0.4"/>
      <value value="0.5"/>
    </enumeratedValueSet>
